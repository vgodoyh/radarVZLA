<x-layouts::admin :title="'Analítica | OVFN'">
    ...
    <main class="access-justice-dashboard ovfn-dashboard">
        ...
        <section id="ovfn-verification-summary" class="ovfn-verification-editor access-justice-card">
            <header class="access-justice-card__header"><div><h2>Total de Verificaciones</h2><p>Dato editorial vigente del módulo OVFN.</p></div></header>
            <div class="ovfn-verification-summary__value"><strong>{{ $currentVerificationTotal?->total ?? 0 }}</strong><span>Actualizado hasta {{ $currentVerificationTotal?->data_date?->format('d/m/Y') ?? 'Sin fecha' }}</span></div>
